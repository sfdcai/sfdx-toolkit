...
export async function POST(req: Request) {
  ...
  if (jobId) {
    const row = db
      .prepare(
        `SELECT c.id, c.user_id as userId, c.project_id as projectId, p.name as projectName, u.tenant_id as tenantId
         FROM comparisons c
  ...
  const rows = db
    .prepare(
      `SELECT c.id, c.user_id as userId, c.project_id as projectId
       FROM comparisons c
       JOIN users u ON c.user_id = u.id
       WHERE u.tenant_id = ?`
    )
  ...
  const result = db
    .prepare(
      `DELETE FROM comparisons WHERE user_id IN (SELECT id FROM users WHERE tenant_id = ?)`
    )
    .run(user.tenantId);
  recordAudit(req as any, user, 'job.clear_all', 'job', undefined, { deleted: result.changes });
  return NextResponse.json({ message: 'Tenant comparison jobs cleared.', deleted: result.changes });
}
